function [img,snr,g,noise_psf] = ismrm_cartesian_iterative_Wave(data,samp_mat,csm,PsfY,reg,replicas)
...
if (isempty(reg))
    ...
    E = @(x,tr) ismrm_encoding_cartesian_Wave(x,csm,samp_mat,PsfY,tr); % QZL wave
    reg_out = [];
else
    % QZL regularization
    reg(reg == 0) = 1; 
    ...
    reg_mask = reg_mask * 1e2;
    % END
    
    % E_base = @(x,tr) ismrm_encoding_cartesian_SENSE(x,csm,samp_mat,tr);
    E_base = @(x,tr) ismrm_encoding_cartesian_Wave(x,csm,samp_mat,PsfY,tr); % QZL wave
