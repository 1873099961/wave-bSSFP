...
pathname = 'D:\MRI(only modify codes)\CSandAI cine rawdata\2.bssfp gfactor\19.20220420_tanyuewu\T1\';
file_Wave = [pathname, 'UID_32239674963147_bwave_locCS4dia_amp0_2ch.raw'];
...
%for i = 1:phs
    ...
    TVWeight = 0.002; 	% Weight for TV penalty0.002
    xfmWeight = 0.00001;	% Weight for Transform L1 penalty0.005
...
%     FT = ESPIRiTp2DFT(mask, N, 1, 2, maps, weights);  %2maps mask and N have same dim.
    ...
    recon2(:,:,i) = im_res;
%end
recon2 = recon2/max(max(max(abs(ifft2c(recon2)))));
as(recon2)